...
describe('Policy Workflow 통합 테스트', () => {
  ...
  it('전체 워크플로우 테스트: Role, Group, User, Policy, State, Permission 생성 및 조회', async () => {
    ...
    // ============================================
    // 3. 사용자 ID 생성 (Supabase Auth 없이 테스트용 ID)
    // ============================================
    console.log('3️⃣ 사용자 ID 생성 중...')
    const timestamp = Date.now()
    for (let i = 1; i <= 3; i++) {
      const userId = `test-user-${i}-${timestamp}`
      userIds.push(userId)
    }
    userIds.forEach((id, idx) => {
      console.log(`   ✅ User ${idx + 1}: ${id}`)
    })
    console.log(`   총 ${userIds.length}명 사용자 생성 완료\n`)
    ...
    console.log(`   버전: ${createdPolicy.version}, 활성화: ${createdPolicy.isActive}\n`)
    ...
    console.log(`Version: ${policyWithDetails?.version}`)
